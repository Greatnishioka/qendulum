...
export function parseArxivFeed(xmlText: string): ArxivFeed {
    ...
    const entries = [...xml.querySelectorAll("entry")].map<ArxivEntry>((entry) => {
        ...
        const doiEl =
            entry.getElementsByTagName("arxiv:doi")[0] ?? entry.getElementsByTagName("doi")[0];
    ...
    return {
        ...
        startIndex: Number(xml.getElementsByTagName("opensearch:startIndex")[0]?.textContent ?? 0),
        itemsPerPage: Number(

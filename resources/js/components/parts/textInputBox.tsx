...
export default function TextInputBox({ inputList, buttonList }: props) {
    ...
    function handleInputChange(index: number, value: string) {
        setFallbackValues((current) => current.map((item, itemIndex) => {
            if (itemIndex === index) {
                return value;
            }
                    ...
                    {buttonList.map((button, index) => {
                        ...
                        return (
                            <button
                                ...
                                onClick={button.isSubmit ? undefined : () => handleButtonClick(button)}
                                type={button.isSubmit ? "submit" : "button"}
                                disabled={button.disabled}
                                className={[
                                    "group relative py-2 pr-4 text-white transition-colors duration-200",
                                    isSearchButton ? "z-40 rounded-r-full bg-(--color-turquoise) disabled:bg-[#666666]" : "",
                                    isFuzzyButton ? "z-30 rounded-r-full bg-[#E7B84A] pl-6 disabled:bg-[#8F8F8F]" : "",
                                    isClearButton ? "z-20 rounded-r-full bg-[#DC3131] pl-6 disabled:bg-[#C1C1C1]" : "",
                                ].join(" ")}
                            >
                                {isSearchButton ? (
                                ...
                                {isFuzzyButton || isClearButton ? (
                                    <div className="pointer-events-none absolute left-0 top-0 h-full aspect-square -translate-x-1/2 overflow-hidden">
                                        <span className={`block h-full aspect-square transition-colors duration-200 ${isFuzzyButton ? "bg-[#E7B84A] group-disabled:bg-[#8F8F8F]" : "bg-[#DC3131] group-disabled:bg-[#C1C1C1]"}`}></span>
                                    </div>
                                ) : null}
                                <div className="flex items-center">
                                    ...
                                    {isSearchButton ? <span className="whitespace-nowrap text-[10px]">検索</span> : null}
                                    {isFuzzyButton ? <span className="whitespace-nowrap text-[10px]">あいまい検索</span> : null}
                                    {isClearButton ? (
